...
class IOInstructionTest {
    ...
    fun testDrawInstructionInvalidASCII() {
        ...
        setup.registers.general[1] = 0x00.toByte()
        setup.registers.general[2] = 0x00.toByte()
    ...
    fun testTimerInstructions() {
        ...
        assertEquals(0x3C, setup.registers.timer.get())

        val readT = ReadTInstruction(0xC2.toByte(), 0x00.toByte())
        readT.execute(setup.registers, setup.memory, setup.screen, setup.keyboard)
        assertEquals(0x3C.toByte(), setup.registers.general[2])
    }
    ...
    fun testConvertToBase10() {
        ...
        setup.registers.address = 0x0100

        val instruction = ConvertToBase10Instruction(0xD0.toByte(), 0x00.toByte())
        instruction.execute(setup.registers, setup.memory, setup.screen, setup.keyboard)

        assertEquals(2, setup.memory.read(0x0100))
        assertEquals(5, setup.memory.read(0x0101))
        assertEquals(5, setup.memory.read(0x0102))
    }
    ...
    fun testConvertByteToAscii() {
        ...
        setup.registers.general[0] = 0x05.toByte()
        ...
        setup.registers.general[2] = 0x0A.toByte()
